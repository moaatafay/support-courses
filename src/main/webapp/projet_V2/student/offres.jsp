    ...
    <section id="courses" class="courses">
      ...
      <div class="container">
        <div class="row" data-aos="zoom-in" data-aos-delay="100">
           <c:if test = "${all_offres_size>=1}">
  		    <c:forEach var="i" begin="0" end="${all_offres_size-1}" step="1"> 
  		    <div class="col-lg-auto col-md-6 d-flex align-items-stretch mb-4">
                  		<div class="course-item">
                    		<img src="${all_offres[i].imageOffre}" class="img-offre" alt="...">
                    			<div class="course-content">
                      				<div class="d-flex justify-content-between align-items-center mb-3">
                        				<h4>${all_offres[i].matiere}</h4>
                        				<p class="price">${all_offres[i].prix} $</p>
                      				</div>
                      					<h3><a href="course-details.html"></a></h3>
                     					 <p>${all_offres[i].infos}.</p>
                     					 <p>${all_offres[i].email}</p>
                     					 <p>${all_offres[i].date}</p>
                      					<div class="trainer d-flex justify-content-between align-items-center">
                        				<div class="trainer-profile d-flex align-items-center">
                        				<form action="show_me_prof_profile" method="post">
                        				<input type="hidden" name="id_offre" value="${all_offres[i].id_offre}">
                          				<button type="submit" style="border:none;background-color:white;">
                          				<img src="${all_offres[i].img}" class="img-fluid mr-2" alt="">
                          				<span>${all_offres[i].name} ${all_offres[i].family_name}</span>
                          			    </button>
                          				</form>
                        				</div>
                        				<div class="trainer-rank d-flex align-items-center">
                          					<span class="mr-2">${all_offres[i].moyenne}</span><i class="far fa-star"></i>
                        				</div>
                      					</div>
                    			</div>
                    			<div class="card-footer bg-transparent border-success">
                      			<form action='contacter_prof' method='post'>
                      			<input type="hidden" name="contacter_prof_id_offre" value="${all_offres[i].id_offre}">
