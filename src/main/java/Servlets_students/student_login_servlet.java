...
@WebServlet("/projet_V2/connexion/student_login")
public class student_login_servlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int operation_status=student_model.verification(request,response);
		HttpSession session=request.getSession();
		if(operation_status!=-1) {
			student student=new student();
			student=student_model.get_student_by_id(operation_status);
			if(student!=null) {
				...
				if(student_model.get_offres_that_i_passsed(operation_status))
				{
				...
				if(student_model.get_notfications(operation_status)) {
				...
				System.out.println("Servlet student is null");
			}
		}
		else {
